...
def raincloud(x, y, markersize, df, width=.7, bw=.25, order=None, hue=None, palette=None, alpha=.8):
    # adding color
    if palette is None:
    ...
    ax = pt.half_violinplot(x=x, y=y, data=df, palette=palette, bw=bw, cut=0., linewidth=0, alpha=alpha,
                            scale="area", width=width, inner=None, orient='v', zorder=1, order=order, hue=hue, dodge=dodge)
    # pt.RainCloud(x=x, y=y, data=df, palette=pal, bw=.25, width_viol=.6, orient='v', zorder=2, order=order, hue=hue, dodge=dodge)

    ax = sns.stripplot(x=x, y=y, data=df, palette=palette, edgecolor="white",  linewidth=1, order=order,
...
def RainCloud(x = None, y = None, hue = None, data = None,
    ...
    ax_violin = pt.half_violinplot(x = x, y = y, hue = hue, data = data,
